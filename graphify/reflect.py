...
`sanad reflect` reads the Q&A memory docs that `sanad save-result` files back
...
because ``sanad export wiki`` deletes every ``wiki/*.md`` on each run — a lessons file
...
def _load_node_community(graph_path: Path, analysis_path: Path,
    ...
    Mirrors how `sanad export wiki` reads graph.json + .graphify_analysis.json +
...
def render_lessons_md(agg: dict[str, Any]) -> str:
    ...
    out: list[str] = [
        "# Lessons",
        "",
        f"_Auto-generated by `sanad reflect` from {agg['total']} session "
...
def lessons_fresh(out_path: Path, memory_dir: Path,
    ...
    Lets ``sanad reflect --if-stale`` skip a redundant run — e.g. when the git
