...
def _index_md(
    ...
    lines += [
        "---",
        "",
        "*Generated by [sanad](https://github.com/safishamsi/graphify)*",
    ]
    return "\n".join(lines)
...
def to_wiki(
    ...
    if not communities:
        raise ValueError(
            ...
            "Run `sanad extract .` or `sanad cluster-only .` first."
        )
    ...
    if not communities:
        raise ValueError(
            ...
            "Re-run `sanad extract .` to regenerate .graphify_analysis.json."
        )
