...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] in ("-h", "--help", "-?"):
        ...
        print("    --graph <path>          path to graph.json (default graphify-out/graph.json)")
        print("  tunnel \"<target>\"       slice a minimal sandbox from the graph and REALLY run it; exit 6 on failure")
        print("    --draft f --at rel      lay a draft file over a path inside the sandbox first")
        print("    --depth N               import-closure depth (default 2)")
        print("    --no-tests / --keep     skip graph-selected tests / keep the sandbox dir")
        print("    --timeout S             per-phase timeout in seconds (default 120)")
        print("  lock-check [file.py]    judge code (or stdin) against the graph's symbol space; exit 5 on violations")
        print("  lock-grammar            emit hard-lock artifact from the graph's symbols")
        print("    --format gbnf|schema    llama.cpp GBNF plan grammar, or enum JSON schema (Ollama)")
        print("    --out <file>            write to a file instead of stdout")
        print("  lock-gen \"<task>\"       generate code under the lock (draft->check->repair loop)")
        print("    --rounds N              max repair rounds (default 3)")
        print("    --backend B / --model M override the LLM backend/model")
        print("  scars [path]            mine git history into danger scores + co-change couples")
        print("    --file <f>              show one file's scars (danger, partners)")
        print("    --min-support N         min shared commits for a couple (default 3)")
        print("    --min-confidence F      min share of edits together (default 0.5)")
        print("    --json                  emit the full scar mapping as JSON")
        ...
        print("    --remember              store a gate-clean answer in sterile memory")
        print("  memory <sub>            gate-guarded permanent knowledge store")
        print("    add \"<insight>\"         admit only if every claim proves (exit 7 otherwise)")
        print("    list [--all] | show <id> | recall \"<q>\" | reverify | forget <id>")
